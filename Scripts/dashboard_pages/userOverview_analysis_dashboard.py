...
def app():
    ...
    #load the data
    data_df=load_data('../data/Clean_data.csv')
    user_info= load_data('../data/application_volumes.csv')
   ...
   #set up the handsets type section
    ...
    with visualization:
        ...
        app = selection_col.selectbox(" Which application do you want to view its total data volumnes?", options=['Social media data vol', 'Google data vol', 'email data vol','Youtube data vol', 'Gaming data vol', 'Netflix data vol', 'Other_apps data vol'], index= 0)
